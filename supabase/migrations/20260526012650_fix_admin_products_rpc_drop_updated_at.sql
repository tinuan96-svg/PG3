/*
  # Fix get_admin_products RPC — remove p.updated_at (column does not exist)

  The products table has no updated_at column. Remove it from the RETURN TABLE
  definition and SELECT list. Use synced_at as the update timestamp instead.
*/

DROP FUNCTION IF EXISTS get_admin_products(text,text,text,uuid,uuid,numeric,numeric,text,text,int,int);
...
BEGIN
  ...
  SELECT up.profile_role INTO v_caller_role
  ...
  SELECT
    ...
    p.approval_status, p.approved_at, p.created_at, p.synced_at,
