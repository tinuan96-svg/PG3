/*
  # Replace product detail RPC to include variants

  Drops and recreates `pocketgrocery_get_store_product_detail` so it returns
  a single jsonb object (instead of SETOF record) that includes a `variants`
  array. Each variant has: id, variant_name, price, discounted_price, stock,
  unit_value, unit_type, sort_order, image_url.
*/
