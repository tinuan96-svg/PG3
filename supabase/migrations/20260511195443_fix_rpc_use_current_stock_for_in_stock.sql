...
/*
  # Fix pocketgrocery_get_store_products RPC to use current_stock for in_stock

  ## Problem
  stock_override = current_stock - max_display_stock (a display cap offset, not actual stock).
  The RPC was using COALESCE(stock_override, p.stock) > 0 to determine in_stock,
  which incorrectly marks products with stock <= max_display_stock as out of stock.
  e.g. a product with stock=3 and max_display_stock=5 gets stock_override=-2 => shows as out of stock.

  ## Fix
  Use sp.current_stock (actual stock from central_inventory or products.stock) for:
  - the returned `stock` value
  - the `in_stock` boolean
  - the `p_in_stock_only` filter
*/
