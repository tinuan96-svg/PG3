/*
  # Fix variant product prices in listing RPCs

  ## Problem
  Products with has_variants=true store price=0 on the base products table.
  Real prices live in the product_variants table. All three listing RPCs were
  returning COALESCE(sp.price_override, p.price) which resolves to 0 for
  variant products, causing "£0.00" to appear on the website.

  ## Fix
  Replace the price expression in all listing RPCs with:
    COALESCE(
      sp.price_override,
      NULLIF(p.price, 0),
  ...
  This means:
  1. Use store-level price override if set
  2. Use base product price if non-zero
  3. Fall back to minimum active variant price

  Same fix applied to original_price for display purposes.

  ## Affected RPCs
  - pocketgrocery_get_store_products
  - get_store_products
  - pocketgrocery_get_store_page_data (filtered_products CTE)
  - pocketgrocery_get_store_page_data_light (filtered_products CTE)
  - pocketgrocery_get_store_product_detail (product_row CTE)
*/

-- Helper macro used inside each CTE — replaces the bare price expression
-- ─── pocketgrocery_get_store_products ─────────────────────────────────────────
...
AS $$
  ...
  base AS (
    ...
    WHERE
      ...
      AND (p_in_stock_only = false OR COALESCE(sp.stock_override, p.stock) > 0)
...
product_row as (
  select
    ...
    COALESCE(
      sp.price_override,
      NULLIF(p.price, 0),
      (SELECT MIN(pv2.price) FROM public.product_variants pv2 WHERE pv2.product_id = p.id AND pv2.is_active = true)
    ) as price,
    COALESCE(
      NULLIF(p.original_price, 0),
      (SELECT MAX(pv2.price) FROM public.product_variants pv2 WHERE pv2.product_id = p.id AND pv2.is_active = true)
    ) as original_price,
    p.discount_percentage,
    coalesce(sp.stock_override, p.stock) as stock,
    (coalesce(sp.stock_override, p.stock) > 0) as in_stock,
