/*
  # Fix pocketgrocery_get_store_products RPC — add SECURITY DEFINER

  ## Problem
  The RPC is LANGUAGE sql without SECURITY DEFINER, so it executes as the
  calling role (anon). It JOINs product_marketing_tags, product_metrics, and
  product_variants — none of which have a SELECT grant for anon — causing
  "permission denied" errors and empty product listings for all public visitors.

  ## Fix
  Recreate the function with SECURITY DEFINER so it always runs as the
  function owner (postgres), which has full access. The function itself already
  has EXECUTE granted to anon and authenticated, and RLS on the underlying
  tables is irrelevant when the function owner is postgres.

  ## Tables touched by this RPC
  stores, store_products, products, product_marketing_tags, product_metrics
*/
...
grant execute on function public.pocketgrocery_get_store_products(text,text,uuid,numeric,numeric,boolean,text,integer,integer) to anon, authenticated;
