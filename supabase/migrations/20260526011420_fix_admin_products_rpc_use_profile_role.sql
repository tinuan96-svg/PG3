/*
  # Fix get_admin_products RPC — use user_profiles for admin check

  The previous version checked auth.jwt() app_metadata for is_admin/role,
  but admin status in this app is stored in user_profiles.profile_role.
  This version checks user_profiles instead, matching how AdminGuard works.
*/
...
DECLARE
  ...
  v_caller_role text;
BEGIN
  -- Check admin status via user_profiles (matches how AdminGuard works in the app)
  SELECT profile_role INTO v_caller_role
  FROM user_profiles
  WHERE id = auth.uid();

  IF v_caller_role IS DISTINCT FROM 'admin' THEN
    -- Also accept JWT app_metadata as fallback (for admin@keralagroceries.com Google user)
    IF NOT (
