/*
  # Fix image resolution across all store product RPCs (v2)

  Drops and recreates RPCs that had mismatched return types.
  Adds resolve_product_image() helper and fixes all four RPCs.
*/

-- Helper: resolves a raw image path to a full URL
...
AS $$
  SELECT CASE
    ...
    ELSE 'https://icnvrpnzjjcbvgcqgiua.supabase.co/storage/v1/object/public/product-images' || raw
  END;
$$;

-- Drop and recreate get_store_products (return type conflict)
  ...
  base as (
    select
      ...
      coalesce(
        ...
        resolve_product_image(p.image_url),
        p.image_thumbnail
      ) as image_url,
...
-- Drop and recreate pocketgrocery_get_store_products
...
-- Fix pocketgrocery_get_store_page_data (returns jsonb, no drop needed)
...
categories_cte as (
  select coalesce(jsonb_agg(
    ...
    ) order by x.sort_order nulls last, x.name
  ), '[]'::jsonb) as categories
  from (
...
-- Fix pocketgrocery_get_store_page_data_light (returns jsonb, no drop needed)
