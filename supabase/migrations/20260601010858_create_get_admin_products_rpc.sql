/*
  # Create get_admin_products RPC

  ## Summary
  Creates the `get_admin_products` stored procedure used by the admin Product Control Center page.
  This RPC replaces the previously dropped version and aligns with the current products table schema.

  ## What This Does
  - Returns paginated, filtered, and sorted product rows for the admin products page
  - Joins to categories to include category_name in results
  - Supports filtering by: search (name/SKU ILIKE), approval_status, centralhub_status (stock),
    brand text ILIKE, category_id, price range
  - Supports sorting by: name, price, stock_qty, created_at, approval_status
  - Includes total_count for pagination (window function)
  - SECURITY DEFINER with is_admin() guard — only admins can call this

  ## Columns Returned
  All products table columns plus:
  - category_name: joined from categories table
  - total_count: total rows matching filters (for pagination)

  ## Security
  - Function is SECURITY DEFINER with explicit search_path
  - Checks is_admin() and returns empty set if not admin
*/
