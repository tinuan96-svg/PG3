...
/*
  # Fix RPC to include allow_backorder in result and treat backorder products as in_stock

  Must drop and recreate because the return type changes (adding allow_backorder column).
*/

DROP FUNCTION IF EXISTS pocketgrocery_get_store_products(text,uuid,uuid,boolean,text,text,integer,integer,text,text);
...
base AS (
  SELECT
    ...
    (COALESCE(sp.current_stock, p.stock, 0) > 0 OR COALESCE(p.allow_backorder, false) = true) AS in_stock,
  ...
  WHERE
    ...
    AND (p_in_stock_only = false
         OR COALESCE(sp.current_stock, p.stock, 0) > 0
         OR COALESCE(p.allow_backorder, false) = true)
