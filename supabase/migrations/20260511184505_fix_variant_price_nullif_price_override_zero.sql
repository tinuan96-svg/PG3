/*
  # Fix variant prices: NULLIF price_override = 0 as well

  The previous fix used COALESCE(sp.price_override, NULLIF(p.price,0), variant_min)
  but store_products.price_override is stored as 0.00 (not NULL) for variant products,
  so COALESCE was resolving to 0.00 immediately.

  Fix: wrap price_override in NULLIF(sp.price_override, 0) so a zero override is
  treated the same as no override, allowing the variant price fallback to activate.

  Expression used in all listing and detail RPCs:
    COALESCE(
      NULLIF(sp.price_override, 0),
      NULLIF(p.price, 0),
      (SELECT MIN(pv.price) FROM product_variants pv WHERE pv.product_id = p.id AND pv.is_active)
    )
*/
